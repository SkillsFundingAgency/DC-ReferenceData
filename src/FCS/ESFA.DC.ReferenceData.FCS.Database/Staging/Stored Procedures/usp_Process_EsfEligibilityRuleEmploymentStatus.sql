CREATE PROCEDURE [Staging].[usp_Process_EsfEligibilityRuleEmploymentStatus]
AS
BEGIN
	SET NOCOUNT ON;
	...
	BEGIN TRY
		...
		MERGE INTO [dbo].[EsfEligibilityRuleEmploymentStatus] AS Target
		USING (
				SELECT   [TenderSpecReference]
						,[LotReference]
						,[Code]
				  FROM [Staging].[EsfEligibilityRuleEmploymentStatus]
			  )
			  AS Source 
