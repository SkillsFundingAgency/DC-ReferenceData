CREATE PROCEDURE [Staging].[usp_Process_EsfEligibilityRuleLocalAuthority]
AS
BEGIN
	SET NOCOUNT ON;
	...
	BEGIN TRY
		...
		MERGE INTO [dbo].[EsfEligibilityRuleLocalAuthority] AS Target
		USING (
				SELECT   [TenderSpecReference]
						,[LotReference]
						,[Code]
				  FROM [Staging].[EsfEligibilityRuleLocalAuthority]
			  )
			  AS Source 
